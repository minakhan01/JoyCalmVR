...
[AddComponentMenu("GoogleVR/MotionHead")]
public class MotionHead : MonoBehaviour {
  ...
  void Awake() {
    MotionViewer.Create();
		MotionViewer.Instance.OnBackButton += HandleOnBackButton;
  }
  ...
  private void UpdateHead() {
    ...
    updated = true;
    MotionViewer.Instance.UpdateState();

    if (trackRotation) {
      var rot = MotionViewer.Instance.HeadPose.Orientation;
      if (target == null) {
    ...
    if (trackPosition) {
      Vector3 pos = MotionViewer.Instance.HeadPose.Position;
      if (target == null) {
